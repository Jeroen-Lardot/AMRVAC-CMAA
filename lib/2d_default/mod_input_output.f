...
contains
  ...
  subroutine read_par_files()
    ...
    namelist /methodlist/ time_stepper,time_integrator, source_split_usr,&
       ...
       gradient_limiter,cada3_radius,loglimit,typeboundspeed, typetvd,&
       typeentropy,entropycoef,typeaverage, typegrad,typediv,typecurl,&
       nxdiffusehllc, flathllc, tvdlfeps,flatcd,flatsh,rk2_alfa,ssprk_order,&
       rk3_switch,imex_switch,small_temperature,small_pressure,small_density,&
        small_values_method, small_values_daverage, check_small_values,&
        trace_small_values, angmomfix, small_values_fix_iw,&
        small_values_use_primitive, schmid_rad1,schmid_rad2, trac
...
'will try default value for qstretch_baselevel in dimension', 2
    ...
    if(.not.phys_energy .and. trac) then
      if (mype==0) write(unitterm, '(A)') &
"Warning, TRAC is not for energy-independent problems, change trac to false."
      trac=.false.
    end if
...
!        call mpistop("Cannot restart with a different physics type")
  ...
  subroutine read_snapshot
    ...
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGslo1,ixGslo2,ixGshi1,ixGshi2) * nws
      allocate(ws(ixGslo1:ixGshi1,ixGslo2:ixGshi2,1:nws))
    end if
    allocate(w_buffer(n_values))
