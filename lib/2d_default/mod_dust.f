...
111   close(unitpar)
  ...
  subroutine dust_check_params()
    ...
    if (gas_mu <= 0.0d0 .and. (dust_method == 'Kwok' .or. dust_method == &
       'sticking')) call mpistop &
       ("Dust error: gas_mu (molecular weight) negative or not set")
    ...
    if (dust_method == 'none') then
       ...
       if (.not. associated(usr_get_3d_dragforce) .or. .not. &
          ...
          mpistop("Dust error:usr_get_3d_dragforce not defined")
    else
       if (any(dust_size < 0.0d0)) call mpistop(&
          "Dust error: any(dust_size < 0) or not set")
       if (any(dust_density < 0.0d0)) call &
          mpistop("Dust error: any(dust_density < 0) or not set")
    end if
  ...
  subroutine dust_get_dt(w, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,ixOmin2,&
    ...
    double precision, dimension(ixImin1:ixImax1,&
       ixImin2:ixImax2)         :: vt2, deltav, tstop, vdust, OmegaK
