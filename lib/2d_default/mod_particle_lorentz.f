...
contains
    ...
    do n = 1, num_particles
      ...
      if (ipe_particle == mype) then
        ...
        if (.not. associated(usr_update_payload)) then
          call Lorentz_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,&
             ps(igrid)%x,x(:,n),v(:,n)*lfac,q(n),m(n),payload,npayload,0.d0)
        else
  ...
  subroutine Lorentz_integrate_particles(end_time)
    ...
    double precision, dimension(ndir) :: b, e, emom, uminus, t_geom, s, udash,&
        tmp, uplus, xcart1, xcart2, ucart2, radmom
    ...
    do iipart=1,nparticles_active_on_mype
      ...
      call get_vec(ep, particle(ipart)%igrid, particle(ipart)%self%x,&
         particle(ipart)%self%time,e)
