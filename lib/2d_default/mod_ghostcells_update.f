    ...
    contains
      ...
      subroutine bc_send_srl
        ...
        if (ipole==0) then
           ...
           if (ipe_neighbor==mype) then
              ixRmin1=ixR_srl_min1(iib1,n_i1);ixRmin2=ixR_srl_min2(iib2,n_i2)
              ixRmax1=ixR_srl_max1(iib1,n_i1);ixRmax2=ixR_srl_max2(iib2,n_i2);
              call pole_copy(psb(ineighbor)%w,ixGmin1,ixGmin2,ixGmax1,ixGmax2,&
                 ixRmin1,ixRmin2,ixRmax1,ixRmax2,psb(igrid)%w,ixGmin1,ixGmin2,&
              ...
              call pole_buffer(pwbuf(ipwbuf)%w,ixSmin1,ixSmin2,ixSmax1,ixSmax2,&
                 ixSmin1,ixSmin2,ixSmax1,ixSmax2,psb(igrid)%w,ixGmin1,ixGmin2,&
                 ixGmax1,ixGmax2,ixSmin1,ixSmin2,ixSmax1,ixSmax2)
              isend_c=isend_c+1
          ...
          if (ipe_neighbor==mype) then
            ...
            do idir=1,ndim
              ...
              psb(igrid)%ws(ixRmin1:ixRmax1,ixRmin2:ixRmax2,&
                 idir) = psb(ineighbor)%ws(ixSmin1:ixSmax1,ixSmin2:ixSmax2,&
                 idir)
            end do
          else
          ...
          if (ipe_neighbor==mype) then
            ...
            do idir=1,ndim
              ...
              call pole_copy_stg(psb(igrid)%ws,ixRmin1,ixRmin2,ixRmax1,ixRmax2,&
                 psb(ineighbor)%ws,ixSmin1,ixSmin2,ixSmax1,ixSmax2,idir)
            end do
          else
             pole_buf%ws=zero
             do idir=1,ndim
              ...
              pole_buf%ws(ixSmin1:ixSmax1,ixSmin2:ixSmax2,&
                 ...
                 1),shape=shape(psb(igrid)%ws(ixSmin1:ixSmax1,ixSmin2:ixSmax2,&
                 idir)))
              ibuf_recv_srl=ibuf_next
              call pole_copy_stg(psb(igrid)%ws,ixRmin1,ixRmin2,ixRmax1,ixRmax2,&
                 pole_buf%ws,ixSmin1,ixSmin2,ixSmax1,ixSmax2,idir)
             end do
          end if
        end if
          ...
          do ic1=1+int((1-i1)/2),2-int((1+i1)/2)
             ...
             if (ipe_neighbor==mype) then ! Same processor
               do idir=1,ndim
                  ...
                  psb(igrid)%ws(ixRmin1:ixRmax1,ixRmin2:ixRmax2,&
                     idir)=psc(ineighbor)%ws(ixSmin1:ixSmax1,ixSmin2:ixSmax2,&
                     idir)
               end do
             ...
             if (ipe_neighbor==mype) then ! Same processor
               do idir=1,ndim
                 ...
                 call pole_copy_stg(psb(igrid)%ws,ixRmin1,ixRmin2,ixRmax1,&
                    ixRmax2,psc(ineighbor)%ws,ixSmin1,ixSmin2,ixSmax1,ixSmax2,&
                    idir)
               end do
             else ! Different processor
           ...
           if (ipole==0) then
              ...
              if (ipe_neighbor==mype) then
                ...
                psc(ineighbor)%w(ixRmin1:ixRmax1,ixRmin2:ixRmax2,&
                   nwhead:nwtail) =psb(igrid)%w(ixSmin1:ixSmax1,&
                ...
                if(stagger_grid) then
                  ...
                  do idir=1,ndim
                    ...
                    sendbuffer_p(ibuf_start:ibuf_next-&
                       1)=reshape(psb(igrid)%ws(ixSmin1:ixSmax1,&
              ...
              if (ipe_neighbor==mype) then
                ...
                call pole_buffer(pwbuf(ipwbuf)%w,ixSmin1,ixSmin2,ixSmax1,&
                   ixSmax2,ixSmin1,ixSmin2,ixSmax1,ixSmax2,psb(igrid)%w,&
                   ixGmin1,ixGmin2,ixGmax1,ixGmax2,ixSmin1,ixSmin2,ixSmax1,&
                   ixSmax2)
                isend_c=isend_c+1
                isend_buf(ipwbuf)=isend_c
          ...
          if(ipe_neighbor==mype) then !! Same processor
            ...
            do idir=1,ndim
              ...
              psc(igrid)%ws(ixRmin1:ixRmax1,ixRmin2:ixRmax2,&
                 idir)=psb(ineighbor)%ws(ixSmin1:ixSmax1,ixSmin2:ixSmax2,idir)
            end do
          else !! Different processor
            do idir=1,ndim
              ixRmin1=ixR_p_stg_min1(idir,inc1)
          ...
          if (ipe_neighbor==mype) then
            ...
            do idir=1,ndim
              ...
              pole_buf%ws(ixRmin1:ixRmax1,ixRmin2:ixRmax2,&
                 ...
                 idir)))
              call pole_copy_stg(psc(igrid)%ws,ixRmin1,ixRmin2,ixRmax1,ixRmax2,&
                 pole_buf%ws,ixRmin1,ixRmin2,ixRmax1,ixRmax2,idir)
              ibuf_recv_p=ibuf_next
            end do
          end if
      ...
      subroutine fix_auxiliary
        ...
        do iigrid=1,igridstail; igrid=igrids(iigrid);
          saveigrid=igrid
          block=>psb(igrid)
          call identifyphysbound(psb(igrid),iib1,iib2)
          ...
          do i1=-1,1
             ...
             call phys_get_aux(.true.,psb(igrid)%w,ps(igrid)%x,ixGmin1,ixGmin2,&
