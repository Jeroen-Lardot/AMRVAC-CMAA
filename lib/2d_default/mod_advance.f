...
contains
  ...
  subroutine advect(idimmin,idimmax)
    ...
    case ("onestep")
       ...
       case ("Forward_Euler")
          call advect1(flux_scheme,one,idimmin,idimmax,global_time,ps1,&
             global_time,ps)

       case ("IMEX_Euler")
          call advect1(flux_scheme,one,idimmin,idimmax,global_time,ps,&
             global_time,ps1)
    ...
    case ("twostep")
       ...
       case ("Predictor_Corrector")
          ...
          ! corrector step
          fix_conserve_at_step = time_advance .and. levmax>levmin
          call advect1(flux_scheme,one,idimmin,idimmax,global_time+half*dt,ps1,&
             global_time,ps)
       ...
       case ("ssprk2")
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,half,idimmin,idimmax,global_time+dt,ps1,&
       ...
       case ("IMEX_Midpoint")
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,half, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,one, idimmin,idimmax,global_time+half*dt,&
             ps2,global_time,ps)
       ...
       case ("IMEX_Trapezoidal")
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,half, idimmin,idimmax,global_time+dt,ps2,&
             global_time+half*dt,ps)
    ...
    case ("threestep")
       ...
       case ("IMEX_ARS3")
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,half, idimmin,idimmax,&
       ...
       case ("IMEX_232")
          ...
          !$OMP END PARALLEL DO
          call getbc(global_time+imex_a21*dt,dt,ps1,1,nwflux+nwaux,&
             phys_req_diagonal)
    ...
    case ("fourstep")
       ...
       case ("rk4")
          ...
          call advect1(flux_scheme,half, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          call advect1(flux_scheme,half, idimmin,idimmax,global_time+half*dt,&
             ps1,global_time,ps2)
          call advect1(flux_scheme,1.0d0, idimmin,idimmax,global_time+half*dt,&
             ps2,global_time,ps3)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
       ...
       case ("jameson")
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,half, idimmin,idimmax,global_time+dt/3.0d0,&
             ps2,global_time,ps1)
          call advect1(flux_scheme,1.0d0, idimmin,idimmax,global_time+half*dt,&
             ps1,global_time,ps)
  ...
  subroutine advect1_grid(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,idimmin,&
    ...
    case ('cd','cd4')
       call centdiff(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
          ixOmin2,ixOmax1,ixOmax2,idimmin,idimmax,qtC,sCT,qt,s,fC,fE,dx1,dx2,&
          x)
    case ('hancock')
       call hancock(qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
          ixOmax1,ixOmax2,idimmin,idimmax,qtC,sCT,qt,s,dx1,dx2,x)
    case ('fd')
       call fd(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
          ixOmax1,ixOmax2,idimmin,idimmax,qtC,sCT,qt,s,fC,fE,dx1,dx2,x)
    case ('tvd')
       call centdiff('cd',qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
          ixOmax1,ixOmax2,idimmin,idimmax,qtC,sCT,qt,s,fC,fE,dx1,dx2,x)
    ...
    if (associated(usr_process_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         call usr_process_grid(igrid,level,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,&
            ixMlo2,ixMhi1,ixMhi2,qt,ps(igrid)%w,ps(igrid)%x)
      end do
      !$OMP END PARALLEL DO
    ...
    if (associated(usr_process_adv_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         call usr_process_adv_grid(igrid,level,ixGlo1,ixGlo2,ixGhi1,ixGhi2,&
            ixMlo1,ixMlo2,ixMhi1,ixMhi2, qt,ps(igrid)%w,ps(igrid)%x)
      end do
      !$OMP END PARALLEL DO
      call getbc(qt,dt,ps,1,nwflux+nwaux,phys_req_diagonal)
    end if
