...
do iigrid=1,igridstail; igrid=igrids(iigrid)
   ...
   ps1(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw)=ps(igrid)%w(ixGlo1:ixGhi1,&
      ixGlo2:ixGhi2,1:nw)
   ...
   if (nwauxio > 0) then
      if (.not. associated(usr_aux_output)) then
         ...
         call usr_aux_output(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1-1,ixMlo2-1,&
            ixMhi1+1,ixMhi2+1, ps1(igrid)%w,ps(igrid)%x,normconv)
      end if
   end if
end do
...
if (saveprim) then
  do iigrid=1,igridstail; igrid=igrids(iigrid)
    ...
    call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1+1,ixGlo2+1,&
       ixGhi1-1,ixGhi2-1,ps1(igrid)%w,ps(igrid)%x)
    if(B0field) then
      ! add background magnetic field B0 to B
      ps1(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         iw_mag(:))=ps1(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
  ...
  do iigrid=1,igridstail; igrid=igrids(iigrid)
    ...
    if (B0field) then
      ! add background magnetic field B0 to B
      if(phys_energy) ps1(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         iw_e)=ps1(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
         iw_e)+0.5d0*sum(ps(igrid)%B0(ixGlo1:ixGhi1,ixGlo2:ixGhi2,:,0)**2,&
         dim=ndim+1) + sum(ps1(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
