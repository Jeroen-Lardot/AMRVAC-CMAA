...
contains
  ...
  subroutine sample_init()
    ...
    if (.not. associated(particles_fill_gridvars)) particles_fill_gridvars => &
    ...
    do n=1,num_particles
      ...
      if(ipe_particle == mype) then
        ...
        if (.not. associated(usr_update_payload)) then
          call sample_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,&
             ps(igrid)%x,x(:,n),v(:,n),q(n),m(n),payload,npayload,0.d0)
        else
          call usr_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,&
             x(:,n),v(:,n),q(n),m(n),payload,npayload,0.d0)
        end if
...
!      particle(ipart)%self%u(1:ndir) = v(1:ndir)
      ...
      if (.not. associated(usr_update_payload)) then
        call sample_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,&
           x,v,0.d0,0.d0,payload,npayload,tlocnew)
      else
        call usr_update_payload(igrid,ps(igrid)%w,pso(igrid)%w,ps(igrid)%x,x,v,&
           0.d0,0.d0,payload,npayload,tlocnew)
      end if
