...
filehead="  itmf,  dt,  <f_i>,  <CW sin theta>,  <Current>,  <Lorenz force>"
  ...
  subroutine advectmf(idimmin,idimmax,qt,qdt)
    ...
    select case (time_stepper)
     case ("onestep")
       call advect1mf(flux_scheme,qdt,one,idimmin,idimmax,qt,ps1,qt,ps)
     case ("twostep")
       ! predictor step
       ...
       ! corrector step
       fix_conserve_at_step = mf_advance .and. levmax>levmin
       call advect1mf(flux_scheme,qdt,one,idimmin,idimmax,qt+half*qdt,ps1,qt,&
          ps)
     case ("threestep")
       ! three step Runge-Kutta in accordance with Gottlieb & Shu 1998
       call advect1mf(flux_scheme,qdt,one,idimmin,idimmax,qt,ps,qt,ps1)
       ...
       call advect1mf(flux_scheme,qdt,0.25d0,idimmin,idimmax,qt+qdt,ps1,&
          qt+dt*0.25d0,ps2)
