...
contains
  ...
  subroutine params_read(files)
    ...
    namelist /wind_list/ rho_bound, kappa_e, &
    R_star, M_star, v_inf, L_bound
...
113    close(unitpar)
  ...
  subroutine initial_conditions(ixI^L, ixO^L, w, x)
    ...
    do ix1 = ixOmin1,ixOmax1
      ...
      if (rhd_energy) w(ix1,:,e_) = read_initial_conditions(r_in,3)/unit_pressure
      w(ix1,:,r_e) = read_initial_conditions(r_in,4)/unit_pressure
      w(ix1,:,i_diff_mg) = read_initial_conditions(r_in,5)/(unit_length**2/unit_time)
    enddo

    where (x(ixO^S,1) .gt. 1.d0)
      w(ixO^S,rho_) = w(ixO^S,rho_)*1.d-2
    ...
    do ix2=ixOmin2,ixOmax2
      ...
      do ix1=ixOmin1,ixOmax1
        if (x(ix^D,1) .lt. 1.d0) then
          w(ix^D,mom(1)) = 1.d0* rand* dexp(2*dpi*(x(ix^D,1)-1.d0)/(0.2d0))
        else
          w(ix^D,mom(1)) = 1.d0* rand* dexp(-2*dpi*(x(ix^D,1)-1.d0)/(0.2d0))
        endif
      enddo
...
! !      T_out = max(4.2d4/unit_temperature, T_out)
  ...
  subroutine multigrid_fixedflux_bc(box, nc, iv, ib, bc_type, bc)
    ...
    do i = 1,nc
      ...
      Diff(i) = 2*box%cc(i,1,mg_iveps)*box%cc(i,2,mg_iveps)/(box%cc(i,1,mg_iveps)+box%cc(i,2,mg_iveps))
      bc(i) = -F_bound/Diff(i)
    enddo
...
!    endif
 ...
 subroutine ceil_diffcoef(w, wCT, x, ixI^L, ixO^L)
  ...
  subroutine update_extravars(igrid,level,ixI^L,ixO^L,qt,w,x)
    ...
    g_rad(ixO^S) = (OPAL(ixO^S))*rad_flux(ixO^S,1)/(const_c/unit_velocity)
    ...
    Lum(ixO^S) = 4*dpi*rad_flux(ixO^S,1)*(x(ixO^S,1))**2
    ...
    *unit_density*unit_velocity/M_sun*year
    ...
    w(ixO^S,i_Lum) = Lum(ixO^S)/(L_bound)
