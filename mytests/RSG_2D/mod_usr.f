...
contains
  ...
  subroutine usr_init()
    ...
    mg%bc(1, mg_iphi)%boundary_cond => multigrid_fixedflux_bc
    ...
    ! Special Opacity
    usr_special_opacity => get_kappa_OPAL
  ...
  subroutine params_read(files)
    ...
    namelist /wind_list/ rho_bound, kappa_e, R_star, M_star, v_inf, L_bound
...
113    close(unitpar)
    ...
    F_bound = L_bound/(4*dpi*(R_star*xprobmin1)**2)
  ...
  subroutine initial_conditions(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
    ...
    integer :: ix1,ix2, seed(1:50)

    do ix1 = ixOmin1,ixOmax1
      r_in = x(ix1,3,1)*R_star*unit_length/R_sun
      ! print*, r_in

      w(ix1,:,rho_) = read_initial_conditions(r_in,2)/unit_density
      w(ix1,:,mom(1)) = 0.d0
      w(ix1,:,mom(2)) = 1.d-1*dsin(30*x(ix1,:,1))*dcos(100*x(ix1,:,2)) !* w(ix1,:,rho_)
      if (rhd_energy) w(ix1,:,e_) = read_initial_conditions(r_in,&
         3)/unit_pressure
      w(ix1,:,r_e) = read_initial_conditions(r_in,4)/unit_pressure
      w(ix1,:,i_diff_mg) = read_initial_conditions(r_in,&
         5)/(unit_length**2/unit_time)
    enddo

    where (x(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1) .gt. 1.d0)
      w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,rho_) = w(ixOmin1:ixOmax1,&
         ixOmin2:ixOmax2,rho_)*1.d-2
      ! w(ixO^S,e_) = w(ixO^S,e_)*1.d-2
    endwhere

    do ix2=ixOmin2,ixOmax2
      seed = 124893 + int(1d3*x(1,ix2,2)+xprobmax2)**2
      ! print*, seed
      ...
      do ix1=ixOmin1,ixOmax1
        if (x(ix1,ix2,1) .lt. 1.d0) then
          w(ix1,ix2,mom(1)) = 1.d0* rand* dexp(2*dpi*(x(ix1,ix2,&
             1)-1.d0)/(0.2d0))
        else
          w(ix1,ix2,mom(1)) = 1.d0* rand* dexp(-2*dpi*(x(ix1,ix2,&
             1)-1.d0)/(0.2d0))
  ...
  function read_initial_conditions(r_in,index) result(var)
    ...
    open(unit=1, file='Dylan_profs/first_profile')
    ! read(1,*) !> header
    ! read(1,*) !>header
    ! read(1,*) !>header
    read(1,*) w !> first line of data
    do ll = 1,1283 !> loop over all lines
      w_mo = w
      read(1,*) w
        if (w(1) .gt. r_in) then
          w_po = w
...
8765 CLOSE(1)
    ...
    if (r_in .ge. 1.d0) var = w_mo(index)
  ...
  subroutine boundary_conditions(qt,ixImin1,ixImin2,ixImax1,ixImax2,ixBmin1,&
    ...
    case(1)
      ...
      do ix1 = ixBmax1,ixBmin1,-1
        do ix2 = ixBmin2,ixBmax2
          w(ix1,ix2,r_e) = w(ix1+2,ix2,r_e) + (x(ix1,ix2,1)-x(ix1+2,ix2,&
             1))*Local_gradE(ix1+1,ix2)
        enddo
      enddo
...
! !      T_out = max(4.2d4/unit_temperature, T_out)
  ...
  subroutine multigrid_fixedflux_bc(box, nc, iv, ib, bc_type, bc)
    use mod_bc_data
    use mod_multigrid_coupling
    ...
    n_bc         = bc_data_ix(rho_, ib)
    ! In which dimension the boundary is
    nb_dim       = mg_neighb_dim(ib)
    ! A list of dimensions *other* than nb_dim
    ixs          = [(i, i=1,ndim-1)]
    ixs(nb_dim:) = ixs(nb_dim:) + 1

    ! Get the coordinates at the cell faces at the boundary
    call mg_get_face_coords(box, ib, nc, rr)

    do i = 1,nc
      ! Diff(i) = box%cc(i,2,mg_iveps)
      Diff(i) = 2*box%cc(i,1,mg_iveps)*box%cc(i,2,mg_iveps)/(box%cc(i,1,&
         mg_iveps)+box%cc(i,2,mg_iveps))
      bc(i) = -F_bound/Diff(i)
    enddo

  end subroutine multigrid_fixedflux_bc
 ...
 subroutine ceil_diffcoef(w, wCT, x, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
  ...
  subroutine update_extravars(igrid,level,ixImin1,ixImin2,ixImax1,ixImax2,&
    ...
    Lum(ixOmin1:ixOmax1,ixOmin2:ixOmax2) = 4*dpi*rad_flux(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2,1)*(x(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1))**2
